<template>
  <div class="container mx-auto py-8">
    <header class="mb-8">
      ...
      <p class="text-gray-600">{{ filteredPosts.length }} post{{ filteredPosts.length !== 1 ? "s" : "" }}</p>
    </header>

    <div class="space-y-8">
      <article v-for="post in filteredPosts" :key="post.id" class="blog-post group cursor-pointer" @click="navigateTo(post.slug)">
        <div class="flex flex-col md:flex-row md:items-center gap-4">
          ...
          <div class="flex items-center">
            <span class="text-gray-400 group-hover:text-primary transition-colors"> Read More → </span>
          </div>
        </div>
      </article>
